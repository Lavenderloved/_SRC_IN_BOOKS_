...
void Skeleton::coverSets(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_voxel, std::vector<int> &treeSet)
{
    ...
    // set neighbors
    // zjiti sousedy x udeleat vzajemny vztah
...
{
    // pro kazdy cover set  zjisti jeho sousedy z velke koule x dopln je do m_neighbors pod ID coversetu
    m_neighbors.clear();
    ...
    for (int y = 0; y < m_neighbors.size(); y++)
    {
        ...
        for (int s = 0; s < m_neighbors.at(y).size(); s++) // for each neighbor CS get ID
        {
            ...
            for (int x = 0; x < m_neighbors.at(c).size(); x++)
            {
                ...
                if (v == y) // if there is x connection
                    used = true;
            }
...
void Skeleton::makeTreeSets(std::vector<std::vector<int>> &segments)
{
    ...
    std::sort(segments.begin(), segments.end(), [](const std::vector<int> &lhs, const std::vector<int> &rhs)
...
void Skeleton::Segmentation(std::vector<int> treeSet, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, std::vector<std::shared_ptr<Segment>> &segments)
{
    ...
    for (int a = 0; a < segments.size(); a++) // for each segment
    {
        ...
        // int parentID = components.at(x)->getcu;
...
void Skeleton::correctSegments(std::vector<std::shared_ptr<Segment>> &input)
{
    ...
    {
        // std::cout<< "komponenta " << x << " pocet cs: " << input.at(x) ->getCoverSets().size() << "\n";
        ...
        //        if(input.at(x)->getLenght() == 0 && childrens.size() == 0  )   // only one cover set without children or
        //        {
        //            // get parent
        //            // join cs from  input.at(x) to parent
        //            // remove children from parent
        //            // remove segment from input
        //
        //                std::vector<int> coverSetsCH = input.at(x)->getCoverSets();
        //                input.at(x)->getParent()->addCoverSet(coverSetsCH);
        //
        //            // remove children from parent
        //            std::vector<std::shared_ptr<Segment>> ch;
        //            std::vector<std::shared_ptr<Segment>> ch_new;
        //            input.at(x)->getParent()->getChildrens(ch);
        //            for (int m=0; m < ch.size(); m++)
        //            {
        //                if(ch.at(m) != input.at(x))
        //                    ch_new.push_back(ch.at(m));
        //            }
        //            input.at(x)->getParent()->setChildrens(ch_new);
...
void Skeleton::setStemTop(std::vector<std::shared_ptr<Segment>> &components)
{
    ...
    for (int q = 0; q < components.size(); q++)
    {
        ...
        {
            if (segmentCSs.at(w) == m_highestVoxel) // mame nejvyssi voxel x take jeho komponentu
            {
                // std::cout<<"nejvyssi voxel\n";
                //  spočitat x nastavit jako knem
                std::vector<int> stemIDs;
...
void Skeleton::setBranches(std::vector<std::shared_ptr<Segment>> &components)
{
    ...
    //        if(components.at(x)->getOrder()!=-1)
    //        {
    //            //get children
    //            std::vector<std::shared_ptr<Segment>> childrens;
    //            components.at(x)->getChildrens(childrens);
    //            int parentOrder = components.at(x)->getOrder();
...
void Skeleton::setIntensity(std::vector<std::shared_ptr<Segment>> &branches, std::vector<std::vector<int>> inliers)
{
    ...
    // pro kazdou vetev zjisti vsechny CS x jejich body
...
{
    // zjistit jestli spolu sousedi CS v cut x pripadne ktere, pro dalsi to same x ulozit
    ...
    // std::cout<< "n1: ";
    // for(int y=0; y < n1.size();y++) // pro vsechny sousedy
    //    std::cout<< n1.at(y) << " ";
    // std::cout<< "\n ";
    ...
    if (outputParts.size() > 1)
        std::sort(outputParts.begin(), outputParts.end(), [](const std::vector<int> &lhs, const std::vector<int> &rhs)
                  { return lhs.size() > rhs.size(); });
