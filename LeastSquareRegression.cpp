...
stred LeastSquaredRegression::kamihoDBH()
{
    ...
    //nejlepši celkove R x  fitovany kruh;
    //  float R=0;
    stred Finalstred = {-1, -1, -1, -1, -0.5};
    float rmseF = 395000;

    startPoint.x = m_circle.x;
    startPoint.y = m_circle.y;
...
//qWarning()<< "        c.x: " << c.x << " c.y: " << c.y << " c.i " << c.i << " c.z " << c.z << " c.r " << c.r  ;
            float d = 0;
            //spocitat RMSE

            for (int v = 0; v < pointIDv.size(); v++) {
                float x2 = (cl_->points.at(v).x - c.x) * (cl_->points.at(v).x - c.x);
                float y2 = (cl_->points.at(v).y - c.y) * (cl_->points.at(v).y - c.y);
...
//qWarning()<< "Finalstred x: " << Finalstred.x << "Finalstred y: " << Finalstred.y << "Finalstred z: " << Finalstred.i << "Finalstred r: " << Finalstred.r;
    return Finalstred;
}
...
//    computing paramters of the fitting circle
    ...
    m_circle = T;
    //qWarning()<< "   algebraic     c.x: " << T.x << " c.y: " << T.y << " c.i " << T.i << " c.z " << T.z << " c.r " << T.r  ;
}

void LeastSquaredRegression::TaubinFit()
{
    /*
        Circle fit to x given set of data points (in 2D)
       ...
       Output:
             ...
             circle.x - the X-coordinate of the center of the fitting circle
             circle.y - the Y-coordinate of the center of the fitting circle
       ...
       It still has x small bias and its statistical accuracy is slightly
       ...
       It provides x very good initial guess for x subsequent geometric fit.
...
//       starting with the given initial circle (initial guess)
    New = {m_circle.x, m_circle.y, m_circle.r, 0, 0, 0, 0};
...
//       check if improvement is gained
    ...
    m_circle = c;
    // qWarning()<< "   geometric    c.x: " << c.x << " c.y: " << c.y << " c.i " << c.i << " c.z " << c.z << " c.r " << c.r  ;
}
