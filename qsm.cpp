...
void CylinderModel::setStemCurve(std::vector<pcl::PointXYZI>& inVector)
{
    for(int q=0; q < inVector.size();q++)
    {
       ...
       // std::cout<< "s.x: " << s.x << " s.y: " << s.y << " s.z: " << s.z << " s.r: "<< s.r<<"\n";
...
void CylinderModel::computeBranches()
{
    ...
    // urcit median value regresnich coef x ty pouzit na vetve
...
{
    //vytvorit novy point s intesity nejbližšího cylinderu ze segmentu x odečíst vzdálenost cylinderu rodiče
...
void CylinderModel::setSegmentParentCylinder(int i)
{
    ...
    // pro kazde dite urcit z connpoint  nejbližší válec x uložit jej.
    
    // pro dany segment najdi vsechny deti, x pro kazde dite pridej connpoint
...
bool CylinderModel::segmentPartition(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointXYZI& u,  float height, bool reverseDirection)
{
   // std::cout<<"segmentPartition\n";
    ...
    // pro kazdy sub cloud spocitat kruh RHT x ziskat stred x prumer na zacatku x na konci
...
bool CylinderModel::logFit(std::vector<pcl::PointXYZI>& inVector, std::vector<pcl::PointXYZI>& outVector)
{
   ...
   // distance += inVector.at(inVector.size()-1).z;
    ...
    // jisti odchylu od modelu x skutecnych hodnot
...
bool CylinderModel::nonLinearRegressionStem(std::vector<pcl::PointXYZI>& inVector)
{
    ...
    // f(x) = x(x^2) + y(x) + c has 3 parameters: x, y, c
    int n = 3;
    ...
    coeff(0) = 1.0;             // initial value for 'x'
    coeff(1) = 1.0;             // initial value for 'y'
    coeff(2) = 1.0;             // initial value for 'c'
    
    //
    // Run the LM optimization
    // Create x LevenbergMarquardt object and pass it the functor.
    //
...
bool CylinderModel::nonLinearRegressionBranches(std::vector<pcl::PointXYZI>& inVector)
{
    ...
    // The LM optimization inputs should not be confused with the x input values.
    Eigen::VectorXf coeff(n);
    coeff(0) = 0.002;             // initial value for 'x'
    coeff(1) = 0.002;             // initial value for 'y'
...
bool CylinderModel::regressionCoeff(Eigen::VectorXf coeff)
{
    ...
    m_coef3.push_back(coeff(2)); // initial value for 'x'
    return true;
}
...
std::vector<pcl::ModelCoefficients::Ptr> CylinderModel::getCylinders(std::vector<pcl::PointXYZI> points, int branchID)
{
    ...
    // pokud moc vybočuje - viz delka  tak tento bod ignorovat, x spojit dalsim.
...
//        // radius
//        int x=i+1;
//        float radius =(points.at(i).intensity + points.at(x).intensity) /2;
//        // height of cylinder
//        float v = pointDistance(points.at(i), points.at(x));
//
...
//        //return;
//        pcl::ModelCoefficients::Ptr coeff(new pcl::ModelCoefficients);
//        coeff->values.push_back(points.at(x).x);
//        coeff->values.push_back(points.at(x).y);
//        coeff->values.push_back(points.at(x).z);
//        coeff->values.push_back(points.at(i).x - points.at(x).x);
//        coeff->values.push_back(points.at(i).y - points.at(x).y);
//        coeff->values.push_back(points.at(i).z - points.at(x).z);
...
void CylinderModel::coefficientData()
{
   ...
   // std::cout << "hodnoty koeficientu x: min:"<< m_coef1.at(0)<< " max: "<< m_coef1.at(m_coef1.size()-1) << " prumer: "<< average1<< " median: "<< median1 << " \n";
    
    std::sort(m_coef2.begin(), m_coef2.end());
    float average2 = accumulate( m_coef2.begin(), m_coef2.end(), 0.0)/m_coef2.size();
    float median2 =m_coef2[m];
    //std::cout << "hodnoty koeficientu y: min:"<< m_coef2.at(0)<< " max: "<< m_coef2.at(m_coef2.size()-1) << " prumer: "<< average2<< " median: "<< median2 << " \n";
