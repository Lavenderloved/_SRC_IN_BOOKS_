...
float GeomCalc::findLongestPerpendicularDistance(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointXYZI &pointA,
...
bool GeomCalc::isAnyEdgeIntersect(pcl::PointCloud<pcl::PointXYZI>::Ptr listOfEdges, pcl::PointXYZI newEdgePtA,
                                  pcl::PointXYZI newEdgePtB)
{
...
bool GeomCalc::isAnyPointInTriangle(pcl::PointCloud<pcl::PointXYZI>::Ptr points, pcl::PointXYZI a, pcl::PointXYZI b,
                                    pcl::PointXYZI c)
{
...
{
    if (GeomCalc::isXYequal(testedPoint, a) != true && GeomCalc::isXYequal(testedPoint, b) != true &&
        GeomCalc::isXYequal(testedPoint, c) != true)
    {
...
float GeomCalc::getTriangleSideRatio(pcl::PointXYZI pointA, pcl::PointXYZI pointB, pcl::PointXYZI pointC)
{
    ...
    if (a > b && a > c)
    {
        sideRatioSum = 1 + (a / b) + (a / c);
    }
    else if (b > a && b > c)
    {
        sideRatioSum = 1 + (b / a) + (b / c);
    }
    else
    {
        sideRatioSum = 1 + (c / a) + (c / b);
    }
...
pcl::PolygonMesh GeomCalc::triangulatePolygon(pcl::PointCloud<pcl::PointXYZI>::Ptr polygon)
{
    ...
    /*
      ...
      // Add the polygon to x list of polygons
...
// CLOUD OPERATIONS
void CloudOperations::cloudXYZItoXYZ(pcl::PointCloud<pcl::PointXYZI>::Ptr cloudXYZI,
...
void CloudOperations::voxelizeCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr output,
...
void TriangulatedPolygon::polygonTriangulation(pcl::PointCloud<pcl::PointXYZI>::Ptr polygon)
{
    ...
    do
    {
        ...
        if (polsize == polygon->points.size())
        {
            break;
        }
