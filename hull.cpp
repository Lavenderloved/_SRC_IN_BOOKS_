...
void ConvexHull::pclCloudToVTKPoints(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, vtkSmartPointer<vtkPoints> pointsVTK,
                                     float newZcoord)
{
...
void ConvexHull::makeConvexHullDelaunay2D(vtkSmartPointer<vtkPoints> input,
                                          pcl::PointCloud<pcl::PointXYZI>::Ptr outputVertices)
{
...
void ConvexHull::toPolygonOrderWithTestToConvexness(pcl::PointCloud<pcl::PointXYZI>::Ptr vertices,
                                                    pcl::PointCloud<pcl::PointXYZI>::Ptr polygon)
{
    ...
    do
    {
        ...
        {
            float beta = GeomCalc::computeClockwiseAngle(polygon->points.at(a - 1), polygon->points.at(a),
                                                         vertices->points.at(i));
            if (beta > alfa)
            {
                alfa = beta;
                iter = i;
...
void ConvexHull::findSecondpointInPolygon(pcl::PointCloud<pcl::PointXYZI>::Ptr vertices,
                                          pcl::PointCloud<pcl::PointXYZI>::Ptr polygon)
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
/* CLASS CONCAVEHULL */
ConcaveHull::ConcaveHull(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, std::string name, float searchDist)
    : Cloud(cloud, name)
{
    std::string hullName = name + "_hull";
    //    QString hullName = QString("%1_hull").arg(name);
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_h(new pcl::PointCloud<pcl::PointXYZI>);
...
void ConcaveHull::edgesBreaking(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr hullCloud,
                                float searchDist)
{
...
pcl::PointXYZI ConcaveHull::returnEdgeBreakingPoint(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointXYZI boda,
                                                    pcl::PointXYZI bodb, float searchDist)
{
    float Amax = 9999;
    pcl::PointXYZI bodx;
    float distAB = GeomCalc::computeDistance2Dxy(boda, bodb);
    for (int g = 0; g < cloud->points.size(); g++)
    {
        ...
        {
            if ((GeomCalc::computeDistance2Dxy(boda, bodc) < searchDist &&
                 GeomCalc::computeDistance2Dxy(bodb, bodc) < distAB) ||
                (GeomCalc::computeDistance2Dxy(bodb, bodc) < searchDist &&
                 GeomCalc::computeDistance2Dxy(boda, bodc) < distAB))
            {
...
Cloud ConcaveHull::getPolygonSwappedZI()
{
    ...
    if (p1.z < p2.z)
    {
        ...
        // QString x = QString(" p1 %1  p2 %2").arg(p1.z).arg(p2.z);
        // QMessageBox::information(0,("WARNING"),x);
    }
    std::string a = m_name + "_swapppedConcavehull";
    //    QString x = QString("%1_swapppedConcavehull").arg(m_name);
...
void ConcaveHull2::setConvexHull(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud)
{
    m_convexhull = cloud;
}

void ConcaveHull2::setSearchLimit(float s)
{
    m_searchLimit = s;
}
...
float ConcaveHull2::computeDistance(pcl::PointXYZI lineA, pcl::PointXYZI lineB, pcl::PointXYZI point)
{
    ...
    // float x = lineA.y - lineB.y;
    // float y = lineB.x - lineA.x;
    // float c = (lineA.x * lineB.y) - (lineB.x * lineA.y);

    // float dist = std::abs(x*point.x + y*point.y + c)/ std::sqrt(x*x + y*y);
...
float ConcaveHull2::computeTwoPointsDist(pcl::PointXYZI lineA, pcl::PointXYZI lineB)
{
    ...
    return std::sqrt((lineA.x - lineB.x) * (lineA.x - lineB.x) + (lineA.y - lineB.y) * (lineA.y - lineB.y) +
                     (lineA.z - lineB.z) * (lineA.z - lineB.z));
}
...
float ConcaveHull2::computeAngle(pcl::PointXYZI &a, pcl::PointXYZI &b, pcl::PointXYZI &c)
{
    ...
    // std::cout<<std::fixed << std::setprecision(3)<< " x.x "<< x.x<< " x.y "<< x.y <<"\n";
    // std::cout<<std::fixed << std::setprecision(3)<< " y.x "<< y.x<< " y.y "<< y.y <<"\n";
    ...
    for (int u = 0; u < hull->points.size(); u++)
    {
        ...
        for (int t = 0; t < pointID.size(); t++)
        {
            ...
            if (hull->points.size() > 2)
            {
                ...
                {
                    bool intersectLine = intersect(hull->points.at(op), hull->points.at(op + 1),
                                                   hull->points.at(hull->points.size() - 1),
                                                   cl->points.at(pointID.at(t)));
...
ConcaveHull2::findPointZ(pcl::PointCloud<pcl::PointXYZI>::Ptr c, pcl::PointCloud<pcl::PointXYZI>::Ptr z)
{
